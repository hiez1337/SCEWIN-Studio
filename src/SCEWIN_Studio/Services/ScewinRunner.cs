        ...
        try
        {
            ...
            if (hJob != IntPtr.Zero)
            {
                ...
                try
                {
                    ...
                    var setOk = SetInformationJobObject(hJob, JobObjectExtendedLimitInformation, infoPtr, (uint)length);
                    if (!setOk)
                    {
                        Trace.WriteLine($"[WARNING] ScewinRunner: SetInformationJobObject returned false (Error: {Marshal.GetLastWin32Error()})");
                    }
                }
                finally
                {
    ...
    private async Task<(bool success, string output, string error)> ExecuteScewinAsync(string scewinPath, string arguments)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                // Drain asynchronous standard output / standard error buffers per MSDN documentation
                process.WaitForExit();
